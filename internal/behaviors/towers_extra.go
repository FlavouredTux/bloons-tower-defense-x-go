...
var sniperTowerForms = map[string]sniperTowerForm{
	...
	// Shotgun_Slug: homing slug that spawns 5 Frag shrapnel on hit.
	// Bloonzooka_Shot: homing slug that spawns a Medium_Explosion AoE on hit.
...
func (b *SniperMonkeyBehavior) activateSniperAbility(inst *engine.Instance, g *engine.Game) bool {
	...
	if formName == "RPG_Strike" {
		// RPG Strike ability: fire a massive RPG projectile toward the nearest bloon.
		// From GMX: creates RPG at tower pos with LP=400, PP=120, scale=2,
		// leadpop=1, camopop=1; alarm[0]=30 (self-destruct after 30 steps).
...
// From GMX RPG object: image_xscale=2, alarm[0]=30; on destroy spawns Medium_Explosion
// with scale=2.5, LP=400, PP=120, leadpop=1, camopop=1.
...
func (b *RPGProjectileBehavior) rpgExplode(inst *engine.Instance, g *engine.Game) {
	...
	if expl != nil {
		...
		expl.Vars["explode_radius"] = 64.0 // scale 2.5 vs Bloonzooka scale 1.4 → ~64px radius
...
func (b *RPGProjectileBehavior) Alarm(inst *engine.Instance, idx int, g *engine.Game) {
	if idx == 0 {
		// Time's up — detonate at current position.
		b.rpgExplode(inst, g)
	}
}

// ShotgunSlugBehavior — homing sniper slug that spawns 5 Frag shrapnel on impact.
// On hitting its target bloon, spawns 5 Frag pieces equally spaced at 72° offsets.
...
// On hitting its target bloon, destroys itself and creates an explosion
// with LP=4, PP=60, AoE radius ~36, lead+camo pop, lives 8 steps.
...
// Travels in a straight line at speed 24, hits bloons in radius 8, destroyed by alarm[0].
...
type monkeySubForm struct {
	...
	TorpoCycle          int    // >0: every TorpoCycle shots also fires a Torpedo
	Projectile          string // object name of the projectile to spawn
...
var monkeySubForms = map[string]monkeySubForm{
	...
	// path 2 (middle) ────────────────────────────────────────────────────────────
	// Support_Sub: submerges — no dart fire, only Sub_Energy decamo pulse every 50 frames.
	"Support_Sub": {Range: 115, Reload: 7, LP: 1, PP: 3, Camo: 1, NoFire: true, BurstReload: 50},
	// support_Sub tier-3 upgrades ──────────────────────────────────────────────────
	// Bloontonium_Reactor: fires sub-energy + bloontonium energy pulse every 43 frames
	...
	// path 3 (right) ─────────────────────────────────────────────────────────────
	"Torpedo_Sub": {Range: 125, Reload: 7, LP: 1, PP: 3, TorpoCycle: 3, Projectile: "Barbed_Dart"},
	// special path ────────────────────────────────────────────────────────────────
...
func (b *MonkeySubBehavior) fireTorpedo(inst *engine.Instance, g *engine.Game, target *engine.Instance, ppbuff float64) {
	torp := g.InstanceMgr.Create("Torpedo", inst.X, inst.Y)
	if torp == nil {
		return
	}
	dx := target.X - inst.X
...
func (b *MonkeySubBehavior) fireSubDart(inst *engine.Instance, g *engine.Game, form monkeySubForm) {
	...
	// Torpedo_Sub: every TorpoCycle shots, also fire a Torpedo
	if form.TorpoCycle > 0 {
		b.cycle++
		if b.cycle >= form.TorpoCycle {
			b.cycle = 0
			b.fireTorpedo(inst, g, target, ppbuff)
		}
	}
...
func RegisterExtraTowerBehaviors(im *engine.InstanceManager) {
	...
	im.RegisterBehavior("Airburst_Dart", func() engine.InstanceBehavior { return &AirburstDartBehavior{SubHomingDartBehavior: SubHomingDartBehavior{hitRadius: 12}} })
